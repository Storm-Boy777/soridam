...
// 관리자 — 기출 보관함 권한 관리 페이지
// 이메일로 사용자 검색 → 권한 부여/회수 → 보유자 목록
// (강의 권한 관리 페이지와 동일한 UX 패턴)

import { useState, useTransition } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { Library, Search, UserCheck, ShieldOff, ShieldCheck } from "lucide-react";
import { toast } from "sonner";
import {
  getExamArchiveAccessStats,
  getExamArchiveAccessUsers,
  searchUserForExamArchive,
  grantExamArchiveAccess,
  revokeExamArchiveAccess,
  bulkRevokeExamArchiveAccess,
} from "@/lib/actions/admin/exam-archive";
import type {
...
type ConfirmState =
  | { kind: "grant"; user: ExamArchiveUserSearchResult }
  | { kind: "revoke"; user: ExamArchiveAccessUser }
  | { kind: "bulk-revoke"; ids: string[]; emails: string }
  | null;

export default function AdminExamArchivePage() {
  const queryClient = useQueryClient();

  const [searchEmail, setSearchEmail] = useState("");
  const [searchResult, setSearchResult] = useState<ExamArchiveUserSearchResult | null>(null);
  ...
  const { data: stats } = useQuery<ExamArchiveAccessStats>({
    queryKey: ["admin-exam-archive-access-stats"],
    queryFn: getExamArchiveAccessStats,
    staleTime: 10_000,
  });

  const { data: users = [], isLoading } = useQuery<ExamArchiveAccessUser[]>({
    queryKey: ["admin-exam-archive-access-users"],
    queryFn: getExamArchiveAccessUsers,
    staleTime: 10_000,
  });

  const grantMutation = useMutation({
    mutationFn: grantExamArchiveAccess,
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({ queryKey: ["admin-exam-archive-access-stats"] });
        queryClient.invalidateQueries({ queryKey: ["admin-exam-archive-access-users"] });
  ...
  const revokeMutation = useMutation({
    mutationFn: (userId: string) => revokeExamArchiveAccess(userId),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({ queryKey: ["admin-exam-archive-access-stats"] });
        queryClient.invalidateQueries({ queryKey: ["admin-exam-archive-access-users"] });
  ...
  const bulkRevokeMutation = useMutation({
    mutationFn: (userIds: string[]) => bulkRevokeExamArchiveAccess(userIds),
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ["admin-exam-archive-access-stats"] });
      queryClient.invalidateQueries({ queryKey: ["admin-exam-archive-access-users"] });
      setSelectedIds(new Set());
  ...
  const handleSearch = () => {
    ...
    setSearchResult(null);
    startSearch(async () => {
      const res = await searchUserForExamArchive(searchEmail);
      if (res.error) {
  ...
  const handleRevoke = (user: ExamArchiveAccessUser) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-foreground">기출 보관함 권한 관리</h1>
        <p className="mt-1 text-xs text-foreground-secondary">
          /reviews 페이지의 「기출 보관함」 탭에 접근 가능한 사용자를 관리합니다.
          관리자와 크레딧 잔액 보유자는 자동으로 접근 가능하며, 그 외 사용자에게 수동 권한을 부여할 때 사용합니다.
        </p>
      </div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span
                    className={`text-xs font-medium px-1.5 py-0.5 rounded-full ${
                      searchResult.has_exam_archive_access
                        ? "bg-primary-50 text-primary-600"
                        : "bg-surface text-foreground-muted"
                    }`}
                  >
                    {searchResult.has_exam_archive_access ? "권한 보유" : "권한 없음"}
                  </span>
                </div>
              </div>
            </div>
                  ...
                  return (
                    <tr
                      ...
                      className={`cursor-pointer border-b border-border last:border-0 transition-colors ${
                        isSelected ? "bg-primary-50/60" : "hover:bg-surface-secondary/50"
                      }`}
                    >
      ...
      <ConfirmDialog
        ...
        title={
          confirmState?.kind === "grant"
            ? confirmState.user.has_exam_archive_access
              ? "기출 보관함 권한을 재부여하시겠습니까?"
              : "기출 보관함 권한을 부여하시겠습니까?"
            : confirmState?.kind === "revoke"
              ? "기출 보관함 권한을 회수하시겠습니까?"
              : confirmState?.kind === "bulk-revoke"
