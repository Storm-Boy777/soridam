...
export default function AdminCoachingAccessPage() {
  ...
  const [searchResult, setSearchResult] = useState<CoachingUserSearchResult | null>(null);
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span
                    className={`text-xs font-medium px-1.5 py-0.5 rounded-full ${
                      searchResult.has_coaching_access
                        ? "bg-primary-50 text-primary-600"
                        : "bg-surface text-foreground-muted"
                    }`}
                  >
                    {searchResult.has_coaching_access ? "권한 보유" : "권한 없음"}
                  </span>
                </div>
              </div>
            </div>
