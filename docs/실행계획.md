...
Step 1: 시험후기 ✅ 완료 (Server Actions only)
    ↓
Step 2: 스크립트+쉐도잉 🔄 UX 고도화 진행 중 (SA+EF 기반 완료, 5-Step 위저드 고도화)
    ↓
...
### Step 2: 스크립트+쉐도잉 🔄 UX 고도화 진행 중

> AI 스크립트 생성 + 패키지(WAV+JSON) + 내 스크립트 관리 + 쉐도잉 훈련.
> 📖 상세 설계: `설계/스크립트.md` (5-Step 위저드 + opic_tips + 4계층 렌더링 확정)
...
| 항목 | 상태 |
|------|------|
| DB 6테이블 (scripts, script_packages, ai_prompt_templates, script_specs, shadowing_sessions, shadowing_evaluations) | ✅ |
| RLS 정책 (본인 CRUD + 프롬프트/규격서 인증 읽기) | ✅ |
| 인덱스 8개 + updated_at 트리거 + Storage 버킷 | ✅ |
| consume_script_credit / refund_script_credit RPC 함수 | ✅ |
| ai_prompt_templates 시드 (System Prompt 2행 — generate/correct) | ✅ |
| script_specs 시드 60행 (소리담 → 4컬럼 분할 마이그레이션) | ✅ |
| TypeScript 타입 + Zod 스키마 (types/scripts.ts, validations/scripts.ts) | ✅ |
| Server Actions 11개 (actions/scripts.ts) | ✅ |
| Edge Function (supabase/functions/scripts/index.ts — generate/correct/refine) | ✅ |
| 클라이언트 → EF 호출 연결 (callEdgeFunction + fire-and-forget + polling) | ✅ |
| 스크립트 허브 3탭 UI (scripts-content.tsx) | ✅ |
| 스크립트 생성 위저드 3단계 (script-wizard.tsx) — Step 1~2 + 기본 생성 | ✅ |
| master_questions 쿼리 추가 (getTopics, getQuestionsByCategory) | ✅ |
| 위저드 Step 1: TopicPagination + QuestionSelector 재활용 (시험후기 컴포넌트 공유) | ✅ |
| 목표 등급 미설정 시 GradeSettingModal 인라인 게이트 | ✅ |
| .env.local에 OpenAI + ElevenLabs API 키 저장 | ✅ |

#### 남은 작업 — UX 고도화

| # | 항목 | 상태 | 설명 |
|---|------|------|------|
| 1 | 카테고리 선택 UI 개선 | ⏳ | 버튼 크기·터치 타깃 확대, 선택 상태 강화 |
| 2 | Step 3: 생성 대기 화면 | ⏳ | opic_tips DB 테이블 생성 + 시드 데이터 + 학습 콘텐츠 카드 로테이션 UI |
| 3 | Step 4: 4계층 렌더링 + T/E/F 색상 | ⏳ | paragraphs>slots>sentences>parts, template=회색/expression=테라코타/filler=뮤트 |
| 4 | Step 4: 재생성 기능 | ⏳ | 스토리 수정 + 확인 모달 + 1크레딧 차감 + generate 재호출 |
| 5 | Step 5: 패키지 생성 EF | ⏳ | scripts-package EF (ElevenLabs TTS + Whisper 타임스탬프) |
| 6 | Step 5: 완료 화면 + 쉐도잉 연결 | ⏳ | Phase 1/2 진행 표시 + 완료 후 쉐도잉 훈련 링크 |
| 7 | 허브 탐색 UI 개선 | ⏳ | 내 스크립트 탭 2단계 필터, 카드 상세 보기 |
...
| **Phase 3 (핵심 모듈)** | **🔄 진행 중** | **Step 2 완료, Step 3 모의고사 대기** |
...
| **Step 2** | **스크립트+쉐도잉** | **🔄 UX 고도화** | **SA + EF** | 기반 완료(6테이블+11SA+3EF). 남은: opic_tips+4계층 렌더링+패키지 EF+재생성 |
...
*최종 업데이트: 2026-02-25*
*상태: Phase 3 Step 2 스크립트+쉐도잉 UX 고도화 진행 중 (5-Step 위저드, opic_tips, 4계층 렌더링, 패키지 EF) → Step 3 모의고사*
