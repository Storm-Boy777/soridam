...
Step 1: 시험후기 ✅ 완료 (Server Actions only)
    ↓
Step 2: 스크립트+쉐도잉 ✅ 완료 (SA + EF + 패키지 EF + 쉐도잉 5단계 UI)
    ↓
...
| Zustand 쉐도잉 스토어 (5단계 상태 + localStorage persist) | ✅ |
| 쉐도잉 UI 11개 컴포넌트 (player/recorder/step-nav + Step 1~5 + 평가 결과/이력) | ✅ |
| 쉐도잉 페이지 재작성 (서버 사전 조회 + 키보드 단축키 1~5) | ✅ |
...
│   ├── step-overlap.tsx                      (Step 2: 오버래핑 + 반복 추적)
│   ├── step-shadow.tsx                       (Step 3: 3라운드 점진 숨김)
...
- **쉐도잉 Step 3**: 3라운드 점진 숨김 (full → first-word → hidden)
- **쉐도잉 Step 5 평가**: 크레딧 1개 차감, 5단어 미만 자동 환불, API 실패 시 환불
...
| **Step 2** | **스크립트+쉐도잉** | **✅ 완료** | **SA + EF** | 6테이블+16SA+6EF(2함수)+위저드+뷰어+쉐도잉 5단계 UI+패키지 생성 EF |
