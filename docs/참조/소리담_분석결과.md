...
3단계 평가 시스템 (V4/V6/V7)으로 구성. **V7이 최신이며 이관 대상**. 62개 체크박스 + TypeScript 평가엔진으로 ACTFL 등급 판정.
...
|------|-----------|-----------|-----------------|
| 평가 방식 | 단순 점수 | 34개 체크박스 + LLM | 62개 체크박스 + TS 평가엔진 |
| 체크박스 | 없음 | 34개 (F1~F5) | 62개 (INT 20 + ADV 42 + AL 12) |
| 레벨 판정 | LLM 직접 | LLM 직접 | **평가엔진 (TypeScript)** |
...
| `mock-test-process-v7` | V7 평가 + 평가엔진 (Q15 시 자동) |
...
| `mock-test-process-v7` | V7 평가 + 평가엔진 |
