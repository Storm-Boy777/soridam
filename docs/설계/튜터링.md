...
> 2026-03-02 소리담 소스코드 분석 결과. 오픽톡닥 구현 시 참고.

### DB 구조 (3세대 병존)

| 세대 | 테이블 | 마이그레이션 | 비고 |
|------|--------|-------------|------|
| **V1** | tutoring_prompts | `20260115_create_tutoring_prompts.sql` (227줄) | 3가지 프롬프트 (generate_template, evaluate_level, evaluate_realtest) |
| **V2** | tutor_skill_analysis, tutor_learning_plan, tutor_drill_progress, tutor_drill_sessions, tutor_drill_attempts | `20251207_create_ai_tutor_tables.sql` (308줄), `20251207_create_ai_tutor_drill_tables.sql` (292줄) | 8대 스킬 기반 진단·훈련 |
| **V3** | ai_tutor_v3_sessions, ai_tutor_v3_question_analysis, ai_tutor_v3_prescriptions, ai_tutor_v3_drill_progress, ai_tutor_v3_prompts | `20251225_create_ai_tutor_v3_schema.sql` (486줄) | 모의고사 기반 진단 → Phase 1/2/3 처방 |

### V2 보조 테이블 (시드 데이터)
- **ai_tutor_skills**: 8대 스킬 + 40개 원인코드 (TF/LC/AC/VC/FL/QM/UN/PR), Gate 기준 G0/G1/G2
- **ai_tutor_characters**: Sori(Practice Coach, Phase 1·3) & Dam(Strategy Designer, Phase 2)
- **ai_tutor_training_modules**: 48개 모듈 (M01_01~M08_06)

### Edge Functions (11개)
- **V1/V2**: tutoring-start(~370줄), tutoring-generate-template-answer(~300줄), tutoring-evaluate-attempt(~300줄), tutoring-generate-tts, tutoring-transcribe, tutoring-get-diagnosis, tutoring-get-progress, tutoring-next-question, tutoring-templates-list, tutoring-get-original-answer
- **V3**: ai-tutor-v3-start-diagnosis, ai-tutor-v3-diagnose-question, ai-tutor-v3-complete-diagnosis, ai-tutor-v3-evaluate, ai-tutor-v3-get-diagnosis, ai-tutor-v3-get-prescription, ai-tutor-v3-get-mock-tests

### 프론트엔드 구조
- **메인 페이지**: `soridam-frontend/app/training-lab/ai-tutor/page.tsx` (~500줄)
- **세션 라우트**: `[sessionId]/diagnosis/page.tsx`, `[sessionId]/training/[progressId]/level/[level]/page.tsx`, `test/page.tsx`
- **상태 관리**: `soridam-frontend/lib/stores/tutoringStore.ts` (~300줄)

### 프롬프트 시스템
1. **generate_template** (RCTF): 슬롯 구조 → 학습자 답변 → 교정 template (paragraphs[]{slots[]})
2. **evaluate_level** (Level 2/3): 템플릿 vs 학습자 답변 비교, 슬롯별 평가
3. **evaluate_realtest** (Level 4): Before/After 체크박스 비교, 개선도 측정

### 핵심 데이터 흐름
```
모의고사 완료 → tutoring-start (유형별 pass_rate 약점 분석)
→ tutoring_sessions + progress 생성
→ Level 1 (드래그앤드롭) → Level 2 (키워드+말하기)
→ Level 3 (기능만+말하기) → Level 4 (Real Test, Before/After 체크박스)
→ 모든 유형 완료 시 세션 completed
```

### 오픽톡닥 이관 시 참고
- ✅ V1/V2의 4레벨 훈련 패턴 참고하되 Level 1을 Read Aloud로 변경
- ✅ RCTF 프롬프트 + scripts 테이블 공유 구조
- ✅ Before/After 체크박스 개선도 추적
- ⚠️ V3의 8대 스킬/40개 원인코드는 복잡도 대비 효과 검토 필요
- ⚠️ Sori & Dam 캐릭터는 오픽톡닥 브랜딩과 별도 결정
- ⚠️ V2/V3 병존으로 코드가 매우 복잡 → 오픽톡닥은 단일 버전으로 단순화
