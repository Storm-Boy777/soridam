...
- 큰 화면에 여러 문항을 동시에 보므로 **간결**하게 씁니다.
- 등급 비특정: 특정 등급(IM/IH/AL)을 언급하지 말고 보편적 답변 방향만 안내합니다.

# 각 질문 가이드 구성
- type_label: 질문 유형 (제공된 type_short_kor 그대로)
- answer_flow: 답변 흐름 2~3단계 (각 6~14자, 토픽 맥락 반영한 짧은 라벨)
- vocab: 추천 어휘·표현 4~6개 — 각 { "en": 영어 표현, "ko": 한글 뜻 }. 이 토픽 답변에 바로 쓸 콜로케이션·구동사·표현 위주.
- example: 답변을 시작하기 좋은 예시 문장 1개 (자연스러운 영어 구어체 한 문장).
  ...
  "questions": [
    {
      ...
      "answer_flow": ["단계1 (6~14자)", "단계2", "(선택)단계3"],
      "vocab": [
        { "en": "영어 표현", "ko": "한글 뜻" }
      ],
      "example": "답변 시작 예시 영어 문장 1개"
    }
...
- vocab은 질문마다 4~6개. 토픽에 바로 쓸 수 있는 실용 표현으로.
- example은 자연스러운 구어체 영어 한 문장.`;
}
